...
app1.controller('ctrl1', function($scope) {
  ...
  $scope.updateValue = function() {
    $scope.calculation = $scope.first + ' + ' + $scope.second +
      " = " + (+$scope.first + +$scope.second); //the starting + is forcing it to be an int. 
	  //scope is all of the values that are connected to the referenced controler. So scope is everything from ctrl1. 
  };
  
  $scope.updatehypotenuse = function() {
    $scope.hypotenuse = Math.sqrt($scope.first) + Math.sqrt($scope.second);
	
};



}); //this is the end of app1 controller. 

classes = ['Math 103','COMM 105','MATH 120B','PHIL 205A','CS 105','CS 146','CS 116','CS 245','RPW 304','ACCT 213','CS 216'];

app1.controller('randomclass', function($scope){
	//classes = ['Math 103','COMM 105','MATH 120B','PHIL 205A','CS 105','CS 146','CS 116','CS 245','RPW 304','ACCT 213','CS 216'];
	
	$scope.class = classes[Math.floor((Math.random() * 11))];
}); //end of ranclass

app1.controller('randomclass2', function($scope){

	
	$scope.class2 = classes[Math.floor((Math.random() * 11))];
}); //end of ranclass
